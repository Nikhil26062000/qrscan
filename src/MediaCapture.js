...
const MediaCapture = () => {
  ...
  const capturePhoto = () => {
    ...
    setMediaSrc(canvas.toDataURL('image/jpeg'));
  };
  ...
  return (
    <div className="media-capture-container">
      <video ref={videoRef} className="media-video" />
      <div className="media-controls">
        <button className="media-button" onClick={() => handleMediaTypeChange('photo')}>Capture Photo</button>
        <button className="media-button" onClick={() => handleMediaTypeChange('video')}>Record Video</button>
        <button className="media-button" onClick={() => handleMediaTypeChange('audio')}>Record Audio</button>
        {mediaType === 'video' && (
          <button className={`media-button ${isRecording ? 'bg-red-500' : 'bg-green-500'}`} onClick={isRecording ? stopRecording : startRecording}>
      ...
      {mediaSrc && (
        <div className="media-display">
          {mediaType === 'photo' && <img src={mediaSrc} alt="Captured" className="media-image" />}
          {(mediaType === 'video' || mediaType === 'audio') && (
            <video src={mediaSrc} controls className="media-video-display" />
          )}
        </div>
      )}
    </div>
